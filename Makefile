...
CMD_TOOLS := $(patsubst src/tool/ggo/%.ggo,%,$(wildcard src/tool/ggo/*.ggo))

# Tool binaries (that don't depend on SDL)
TOOL_EXECS := $(patsubst src/tool/%.tool.c,$(BIN_DIR)/sk.%,$(wildcard src/tool/*.tool.c))
...
$(GEN_DIR)/%.cmdl.c $(GEN_DIR)/%.cmdl.h: src/tool/ggo/%.ggo | $(GEN_DIR)
...
$(BIN_DIR)/sk.coeff $(BIN_DIR)/sk.compress $(BIN_DIR)/sk.adsrgen $(BIN_DIR)/sk.negate $(BIN_DIR)/sk.encode $(BIN_DIR)/sk.envelope $(BIN_DIR)/sk.view $(BIN_DIR)/sk.delay $(BIN_DIR)/sk.lpf $(BIN_DIR)/sk.hpf: $(OBJ_DIR)/generic_process.o $(OBJ_DIR)/sk_stdins.o

# Generic tool compilation
$(OBJ_DIR)/%.tool.o: src/tool/%.tool.c | $(OBJ_DIR)
	$(CC) $(CFLAGS) -I$(GEN_DIR) -c $< -o $@ $(LDFLAGS)
$(TOOL_EXECS): $(BIN_DIR)/sk.% : $(OBJ_DIR)/%.tool.o | $(BIN_DIR)
	$(CC) $^ -o $@ $(LDFLAGS)

# Special handling for SDL requirement
$(OBJ_DIR)/view.tool.sdl.o: src/tool/view.tool.sdl.c | $(OBJ_DIR)
	$(CC) $(CFLAGS) -c $^ -o $@ $(LDFLAGS) $(SDL_CFLAGS)
