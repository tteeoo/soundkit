...
/* Effect Properties */
#define LPF_BIAS            100.0f    /* Higher values means more bias towards the low pass filter (the low pass filter will be more audible). Lower values means more bias towards the echo. Must be between 0 and 1. */
...
int main(int argc, char** argv)
{
	...
	ma_result result = ma_waveform_init(&config, &g_waveform);
	if (result != MA_SUCCESS) {
		return -1;
	}
        ...
        ma_node_attach_output_bus(&g_splitterNode, 2, &g_waveform, 0);
    }
